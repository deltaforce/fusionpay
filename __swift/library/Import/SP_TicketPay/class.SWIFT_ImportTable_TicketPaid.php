...
class SWIFT_ImportTable_TicketPaid extends SWIFT_ImportTable {
	...
	public function Import() {
	   ...
	   (select tp.pkgid from ".TABLE_PREFIX."sp_packages tp
	     where tp.title = uo.packagetype and tp.description = 'Imported from TicketPay'
	       and uo.package = 'tickets' and uo.packageprice = tp.price
	       and uo.amount = tp.tickets and tp.enabled = 1
		   limit 1
	   ) as packageid,
			date_purchase, packageprice from ".TABLE_PREFIX."userorders uo where package = 'tickets' and status = 'Paid'
		...
		foreach ($RecContainer as $recData) {
			...
			$this->Database->AutoExecute(TABLE_PREFIX."sp_user_payments",
				array(  'userid' =>  $_newUserID,
						'minutes' => 0,
						'tickets' => $recData['amount'],
						'rem_minutes' => 0,
						'rem_tickets' => $recData['amount'],
						'paidby' => 'TicketPay',
	...
	protected function GetTotal() {
		...
		$_countContainer = $this->DatabaseImport->QueryFetch("SELECT COUNT(*) AS totalitems FROM ".TABLE_PREFIX."userorders ".
				"where package = 'tickets' and status = 'Paid'");
