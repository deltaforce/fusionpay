...
class SWIFT_ImportTable_StaffPaid extends SWIFT_ImportTable {
	private $myTableName;
	private $mySortColumn = "ticketid, timetrackid, staffid";
	...
	public function __construct(SWIFT_ImportManager $_SWIFT_ImportManagerObject) {
		...
		$this->myTableName = TABLE_PREFIX . "sp_staff_paid";
	...
	public function Import() {
		...
		$this->ImportManager->GetImportRegistry()->GetNonCached('staff');
		$this->ImportManager->GetImportRegistry()->GetNonCached('timetrack');
		...
		foreach ($RecContainer as $recID => $recData) {
			...
			$_newTTID = $this->ImportManager->GetImportRegistry()->GetKey('timetrack', $recData['timetrackid']);

			$this->Database->AutoExecute($this->myTableName,
				array(	'ticketid' =>  $recData['ticketid'],
						'timetrackid' =>  $_newTTID,
						'staffid' =>  $_newStaffID,
